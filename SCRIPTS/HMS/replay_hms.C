void replay_hms(Int_t RunNumber=0, Int_t MaxEvent=0,const char* ftype="dc_calib") {
  ...
  gHcParms->AddString("g_ctp_database_filename", "DEUTERON_ANALYSIS/DBASE/HMS/STD/standard.database");
  ...
  ifstream bcmFile;
  TString bcmParamFile = Form("DEUTERON_ANALYSIS/PARAM/HMS/BCM/bcmcurrent_%d.param",  RunNumber);
...
// Add Ideal Beam Apparatus
 ...
 // Define output DEF-file 
  ...
  // Define cuts file
  //  DefTreeFile=Form("DEUTERON_ANALYSIS/DEF-files/HMS/%s_cuts.def",ftype);
