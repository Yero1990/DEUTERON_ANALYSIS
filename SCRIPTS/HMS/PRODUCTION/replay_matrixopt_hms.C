void replay_matrixopt_hms(Int_t RunNumber=0, Int_t MaxEvent=0) {
  ...
  const char* ROOTFileNamePattern = "ROOTfiles/hms_replay_matrix_%d.root";
...
// Add Ideal Beam Apparatus
 ...
 analyzer->SetOdefFile("DEF-files/HMS/PRODUCTION/matrix_opt.def");
 // Define cuts file
 ...
 analyzer->SetSummaryFile(Form("REPORT_OUTPUT/HMS/PRODUCTION/summary_scalercut_%d_%d.report", RunNumber, MaxEvent));    // optional
 // Start the actual analysis.
 analyzer->Process(run);
 // Create report file from template.
 analyzer->PrintReport("TEMPLATES/HMS/PRODUCTION/hstackana_production.template",
 		       Form("REPORT_OUTPUT/HMS/PRODUCTION/replay_hms_matrixopt_%d_%d.report", RunNumber, MaxEvent));
