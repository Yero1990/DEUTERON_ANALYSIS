...
void calc_hProt()
{
  ...
  while(getline(ifs, rline))                                                                                                                                         
	...
	hist[index][1] = new TH1F(Form("meas_P: g%d", index+1), Form("Run g%d", index+1), 100, 1.5, 4);  
      ...
      hist[index][2] = new TH1F(Form("pDiff: g%d",index+1), Form("Run g%d: HMS Perc. Dev. P_{meas}", index+1), 100, -4, 4);
	...
	if(TMath::Abs(theta_p)<100 && TMath::Abs(hmsP_calc)<100 && leaf_var[2]>-100 && leaf_var[2]<100. && leaf_var[3] > -0.08 && leaf_var[3] < 0.15 && abs(leaf_var[4]) < 8. )
	  {
	    ...
	    hist[index][2]->Fill((hmsP_calc - leaf_var[2]) / leaf_var[2]*100);  //HMS (Calculated - Measured)/Measured Momentum (% deviation from measured)
	  }
	hist[index][3]->Fill(leaf_var[3]);
      ...
      yRes_arr[index] = mu_Res_fit;          //mean fit % deviation from meas. momentum
