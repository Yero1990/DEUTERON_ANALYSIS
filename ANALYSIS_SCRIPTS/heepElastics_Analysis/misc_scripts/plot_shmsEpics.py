...
df1 = pd.read_csv('shms_EPICS.csv', parse_dates=False)
...
df1.plot(kind='scatter', x='Run', y='HB_set', edgecolor='black', c='none', marker='s', label='HB-set', ax=ax)
...
df1.plot(kind='scatter', x='Run', y='D_set',  edgecolor='black', c='none', marker='p', label='D-set', ax=ax)

plt.xlabel('Run')
plt.ylabel('Magnet Current [A]')
plt.title('SHMS Magnet Set Currents')
