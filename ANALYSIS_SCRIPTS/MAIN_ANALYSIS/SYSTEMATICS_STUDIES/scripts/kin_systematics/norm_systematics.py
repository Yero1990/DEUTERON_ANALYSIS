...
def get_trkEff_syst(pm_set=0, model='', data_set=0):
    ...
    print('df_htrk=',df_htrk)                                                                                                                                                 
    print('df_etrk=',df_etrk)       

    #Find the minimum non-zero errors (this will be assigend as error if the error is zero)
    df_htrack_min = np.min(np.abs(df_htrk[np.nonzero(df_htrk)]))
    df_etrack_min = np.min(np.abs(df_etrk[np.nonzero(df_etrk)]))

    df_htrk[df_htrk==0] = -df_htrack_min
    df_etrk[df_etrk==0] = -df_etrack_min 
...
def get_filenames(pm_set, model, data_set):
    ...
    if(pm_set==80):
        ...
        final_Xsec_file = final_Xsec_dir+'pm%i_laget_bc_corr_set%i.txt'%(pm_set, data_set)
        #Check if file to store systematics exists, else create it
        if not os.path.exists(avg_kin_file):
            sh.copyfile(init_file, avg_kin_file)
