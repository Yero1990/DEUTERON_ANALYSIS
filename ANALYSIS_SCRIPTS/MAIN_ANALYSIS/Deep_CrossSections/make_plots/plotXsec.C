...
//Read ROOTfiles
  TString pwia_fname = Form("pm80/Xsec_pm%d_lagetpwia_dataset%d.root", pm, set);
  TString fsi_fname = Form("pm80/Xsec_pm%d_lagetfsi_dataset%d.root", pm, set);
...
for(int ix_bin=1; ix_bin<=thnq_bins; ix_bin++)
  {
    ...
    //data_thnq_bins[ix_bin-1]->Divide(simc_pwia_thnq_bins[ix_bin-1]);
    //data_thnq_bins[ix_bin-1]->Divide(simc_fsi_thnq_bins[ix_bin-1]);
    //data_thnq_bins[ix_bin-1]->Draw("E");
    //gPad->SetLogy();
