...
'''
This code assumes the systematics have been written to the average kin. file, and reads thses files that have '_systematics.txt' attached
in their name. This code should only be run after running the code to calculate the total systematic error.
...
if pm_set == 80:     
    output_file = './%s/pm%i_laget_bc_corr_systematics.txt'%(sys_ext, pm_set)                                      #create output file to write avg kin                                   
    ...
    fk = B.get_file('../average_kinematics/%s/pm%i_fsi_norad_avgkin_systematics.txt'%(sys_ext, pm_set)) #Load avg kin. with systematics
else:
    output_file = './%s/pm%i_laget_bc_corr_set%i_systematics.txt'%(sys_ext, pm_set, data_set)
    ...
    fk = B.get_file('../average_kinematics/%s/pm%i_fsi_norad_avgkin_set%i_systematics.txt'%(sys_ext, pm_set, data_set)) #Load avg kin. with systematics
...
#Get Relative Systematic Error, (dsig / sig)
tot_syst_rel_err = B.get_data(fk, 'dsig_tot_syst')  #systematic error is in fractional form df/f
...
for i, ib in enumerate(ib_t):
    ...
    fsiRC_dataXsec_fsibc_corr_syst_err = -1        #systematic error
    fsiRC_dataXsec_fsibc_corr_total_err = -1       #total error, (stats**2 + syst**2) added in quadrature
    ...
    red_dataXsec_err = -1.
    red_dataXsec_syst_err = -1.           #reduced dataXsec systematic error
    red_dataXsec_total_err = -1.          #reduced dataXsec total error
    ...
    if fsiXsec_theory[i]>0. and fsiXsec_avg[i]>0.:
        ...
        if fsiRC_dataXsec_avg[i]>0.:
            ...
            fsiRC_dataXsec_fsibc_corr_syst_err = np.sqrt((tot_syst_rel_err[i]*fsiRC_dataXsec_fsibc_corr)**2)
            fsiRC_dataXsec_fsibc_corr_total_err = np.sqrt(fsiRC_dataXsec_fsibc_corr_err**2 + (tot_syst_rel_err[i]*fsiRC_dataXsec_fsibc_corr)**2) 
            ...
            if fsi_Ksig_cc1[i]>0.:
                ...
                red_dataXsec_syst_err = fsiRC_dataXsec_fsibc_corr_syst_err/fsi_Ksig_cc1[i]
                red_dataXsec_total_err = fsiRC_dataXsec_fsibc_corr_total_err/fsi_Ksig_cc1[i]
