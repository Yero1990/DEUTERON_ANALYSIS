...
#fULL cORR.
f = B.get_file('Heep_yield_fullcorrections.data')
...
B.plot_exp(shms_rate, R, R_err, color='red', marker='o', label='proton absorption + target boiling ')

'''
#----------
#HMS Delta/SHMS Delta CUTS
f = B.get_file('Heep_yield_pAbsCorr.data')
Run = B.get_data(f, 'Run')
dataY = B.get_data(f, 'dataY')    
dataY_err = B.get_data(f, 'dataY_err')     #these are absolute errors, sqrt(N)
simcY = B.get_data(f, 'simcY')
...
B.plot_exp(shms_rate, R, R_err, color='blue', marker='^', label='proton absorption')

#---------------------------

#----------                                                                                                      
#HMS Delta/SHMS Delta CUTS                                                                                                                   
f = B.get_file('Heep_yield_NOcorrections.data')                                                                                                             
Run = B.get_data(f, 'Run')                                                                                                                            
dataY = B.get_data(f, 'dataY')                                                                                                                               
dataY_err = B.get_data(f, 'dataY_err')    #these are absolute errors, sqrt(N)                                                                                          
simcY = B.get_data(f, 'simcY')                                                                                                                                      
...
R = dataY / simcY                                                                                                                                           
R_err = np.sqrt( dataY_err**2/simcY**2 + dataY**2*simcY_err**2/simcY**4 )                                                                       
B.plot_exp(shms_rate, R, R_err, color='black', marker='s', label='No Corrections') 
#---------------------------      
'''
