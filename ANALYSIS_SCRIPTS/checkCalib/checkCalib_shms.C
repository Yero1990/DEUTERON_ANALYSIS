...
void checkCalib_shms(string detector, int runNUM)
{

  TString spec, pfx, pCER;
  
  spec = "SHMS";
  pfx = "P";
 
  //which SHMS Gas Cherenkov to Use?
  pCER = "HGC";  //Heavy Gas Cherenkov
  //pCER = "NGC";  //Heavy Gas Cherenkov


  
  //the user may input the following for detector: "dc", "cal" or "hod"
  ...
  TString ndc_dist;
  TString ndc_nhit;
  TString ndc_res = pfx+".dc.residualExclPlane";
  
  TString nhod_beta = pfx+".hod.beta";
  TString nhod_beta_notrk = pfx+"hod.betanotrack";
  TString nhod_xtrack;
  TString nhod_ytrack;

  TString ncer_npe;

  if(spec=="HMS"){ ncer_npe = "H.cer.npeSum";}
  
  if(spec=="SHMS")
    {
      if(pCER=="HGC"){ncer_npe = "P.hgcer.npeSum";}
      if(pCER=="NGC"){ncer_npe = "P.ngcer.npeSum";}
    }

  TString ncal_etot = pfx+".cal.etot";
  TString ncal_etrknorm = pfx+".cal.etracknorm";
  TString ncal_xtrack = pfx+".cal.xtrack";
  TString ncal_ytrack = pfx+".cal.ytrack";
  ...
  Double_t x[dcPLANES];
      ...
      for (Int_t npl = 0; npl < dcPLANES; npl++ )
	{
	  ...
	  T->SetBranchAddress(pfx+".dc.residualExclPlane", &dc_res[0]);
